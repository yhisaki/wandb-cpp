...
class AsyncLoggingWorker {
 private:
  ...
  // config
  std::queue<object::PyDictItem> config_buffer_;

  // summary
  std::queue<object::PyDictItem> summary_buffer_;
 ...
 public:
  ...
  void append_config(object::PyDictItem&& config);

  bool is_summary_buffer_empty();
  void append_summary(const object::PyDictItem& summary);
  void append_summary(object::PyDictItem&& summary);
